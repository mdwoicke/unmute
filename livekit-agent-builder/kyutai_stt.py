        ...
        try:
            async for message_bytes in ws:
                ...
                if msg_type == "Word":
                    ...
                    # Require sustained high pause probability before finalizing.
                    # Each step is ~80ms, so 8 consecutive steps = ~640ms of silence.
                    # This prevents mid-sentence pauses from triggering FINAL_TRANSCRIPT.
                    if pause_prob > 0.7:
                        self._high_pause_count = getattr(self, '_high_pause_count', 0) + 1
                    else:
                        self._high_pause_count = 0

                    if self._speaking and self._high_pause_count >= 8 and self._current_text:
                        self._speaking = False
                        self._high_pause_count = 0
                        self._event_ch.send_nowait(
                            stt.SpeechEvent(
