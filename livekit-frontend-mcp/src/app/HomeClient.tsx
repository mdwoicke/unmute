  ...
  if (!token) {
    return (
      <div className="container">
        <h1>Unmute LiveKit MCP</h1>
        <p className="subtitle">
          Voice AI with MCP tool calling. Try: &quot;What&apos;s the weather in Beverly Hills?&quot;
        </p>
        ...
        <p className="subtitle" style={{ fontSize: "0.75rem" }}>
          Powered by Kyutai STT/TTS + Qwen 3 4B (LM Studio) + LiveKit + MCP - Moshi Full Duplex - Qwen 3 4B
        </p>
      </div>
    );
  }
        ...
        {transcripts.map((entry, i) => (
          <div key={i} className="transcript-entry">
            ...
            {entry.isFunctionCall && (
              <div className="weather-badge">MCP Tool Call</div>
            )}
          </div>
        ))}
      </div>
