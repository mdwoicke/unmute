...
const UnmuteConfigurator = ({
  ...
  const [collapsedCategories, setCollapsedCategories] = useState<Set<string>>(
    new Set(["Community Voices"])
  );

  const groupedVoices = useMemo(() => {
    if (!voices) return null;
    const groups: { category: string; voices: VoiceSample[] }[] = [];
    const categoryMap = new Map<string, VoiceSample[]>();
    ...
    for (const voice of voices) {
      ...
      categoryMap.get(cat)!.push(voice);
    }

    for (const cat of categoryOrder) {
      groups.push({ category: cat, voices: categoryMap.get(cat)! });
    }
    return groups;
  }, [voices]);
      ...
      <div className="w-full md:bg-gray flex flex-col items-center">
        <div className="w-full max-w-6xl grid grid-flow-row grid-cols-1 md:grid-cols-2 gap-3 p-3">
          <div className="flex flex-col gap-3 max-h-[60vh] overflow-y-auto pr-1">
            {groupedVoices &&
              groupedVoices.map(({ category, voices: categoryVoices }) => (
                <div key={category}>
                  <button
                    onClick={() => toggleCategory(category)}
                    className="flex items-center gap-1 text-lightgray text-xs uppercase tracking-wider mb-2 cursor-pointer hover:text-white transition-colors"
                  >
